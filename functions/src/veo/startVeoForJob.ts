...
export async function startVeoForJobCore(uid: string, jobId: string): Promise<{status: string; finalVideoUrl?: string}> {
  ...
  // Build JSON prompt from category-driven structured templates (commercial)
  ...
  let jsonPrompt = buildProductPrompt(brief as any);
  if (archetype === "cory") {
    // Creative animation flavor
    (brief as any).desiredPerception = ["playful", "energetic"];
    (brief as any).styleWords = ["animated short", "cartoon style", "pixar-like"];
    jsonPrompt = buildProductPrompt(brief as any);
    (jsonPrompt as any).camera = "wide to dolly-in, dynamic";
    (jsonPrompt as any).lighting = "warm energetic";
    (jsonPrompt as any).palette = ["red", "yellow", "green"];
    (jsonPrompt as any).dialogue = [(job?.brief?.dialogue || "") || (job?.debug?.preferredDialogue || null)].filter(Boolean);
  } else if (archetype === "rufus") {
    // Realistic flavor
    (brief as any).desiredPerception = ["authentic", "relatable"];
    (brief as any).styleWords = ["photo-realistic", "natural lighting", "soft fur texture"];
    jsonPrompt = buildProductPrompt(brief as any);
    (jsonPrompt as any).camera = "slow pan, eye-level";
    (jsonPrompt as any).lighting = "golden hour, natural warm";
    (jsonPrompt as any).palette = ["earth brown", "deep green", "warm gold"];
    (jsonPrompt as any).dialogue = [(job?.debug?.preferredDialogue || null)].filter(Boolean);
  } else if (archetype === "scratch") {
    // Generic scaffold; allow brand defaults
    jsonPrompt = buildProductPrompt(brief as any);
  }
  ...
  if (archetype === "cory") {
    ...
    modelName = normalizeModel(job.model as string || "veo-3.0-fast-generate-preview");
  }

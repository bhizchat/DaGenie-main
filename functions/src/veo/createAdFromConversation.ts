...
export interface CreateFromConversationInput {
  ...
  archetype?: "scratch" | "rufus" | "cory" | "commercial";
}
...
export const createAdFromConversation = onCall({
  ...
  await jobRef.set({
    uid, status: "structuring", createdAt: Timestamp.now(), updatedAt: Timestamp.now(), archetype: body.archetype || "commercial",
  }, {merge: true});
  ...
  // Infer format and build prompt (Commercial remains existing path; others get archetype tag persisted now)

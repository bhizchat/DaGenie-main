    ...
    try {
      ...
      for (const s of scenes) {
        ...
        try {
          ...
          // Do not abort the whole batch; skip this scene and continue
          continue;
        }
        ...
        if (forceFlux) {
          try {
            ...
            // If we only have a gs:// reference but not bytes, attempt to sign a short-lived HTTPS URL
            let inputImage: any = replicateInputImage || urls[0];
            if (!replicateInputImage && typeof urls[0] === "string" && urls[0].startsWith("gs://")) {
              try {
              ...
              } catch { /* best-effort; fall back to original */ }
            }

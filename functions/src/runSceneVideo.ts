  ...
  .https.onRequest(async (req, res) => {
    try {
      ...
      await sceneRef.update({
        ...
        await sceneRef.set({ video: { ...(data.video || {}), status: "running", provider: provider || null }, videoStatus: "running", updatedAt: Timestamp.now() }, {merge: true});
      });
      ...
      // If no output yet, pretend we produced one (scaffold)
      if (!outputUrl) {
        throw new Error("provider_no_output_url");
      }

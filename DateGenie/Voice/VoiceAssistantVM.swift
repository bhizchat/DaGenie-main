...
final class VoiceAssistantVM: ObservableObject {
	...
	private func buildContextWordBoost() -> [String] {
		...
		let tokens = tokenizeLettersAndDigitsAllowingHyphen(lastAssistant)
		var words: [String] = []
		for t in tokens {
			let s = String(t)
	...
	private func buildContextKeyterms() -> [String] {
		...
		@inline(__always)
		func isUppercaseToken(_ s: String) -> Bool {
			return s.first?.isUppercase == true && s.count >= 3 && s.count <= 30
		}
		...
		let userTokens = tokenizeLettersAndDigitsAllowingHyphen(lastUser)
		for t in userTokens {
			let s = String(t)
			if s.first?.isUppercase == true && s.count >= 3 && s.count <= 30 { terms.append(s) }
		}
		...
		if !partial.isEmpty {
			let ptoks = tokenizeLettersAndDigitsAllowingHyphen(partial)
			for t in ptoks {
				let s = String(t)
				if s.first?.isUppercase == true && s.count >= 3 && s.count <= 30 { terms.append(s) }
			}
		...
		func addLowercaseNouns(from text: String) {
			let toks = tokenizeLettersAndDigitsAllowingHyphen(text.lowercased())
			for t in toks {
				let s = String(t)
	...
	private func isSeparator(_ ch: Character, allowHyphen: Character) -> Bool {
    if ch == allowHyphen { return false }
    guard let scalar = "\(ch)".unicodeScalars.first else { return true }
    let letters = CharacterSet.letters
    let digits  = CharacterSet.decimalDigits
    return !(letters.contains(scalar) || digits.contains(scalar))
}

	private func tokenizeLettersAndDigitsAllowingHyphen(_ text: String) -> [Substring] {
		let letters = CharacterSet.letters
		let digits  = CharacterSet.decimalDigits
		...
		while i < text.endIndex {
			...
			} else if let scalar = "\(ch)".unicodeScalars.first, letters.contains(scalar) || digits.contains(scalar) {
				if runStart == nil { runStart = i }
			} else {
				if let s = runStart { tokens.append(text[s..<i]); runStart = nil }
			}
			i = text.index(after: i)
		}
		if let s = runStart { tokens.append(text[s..<text.endIndex]) }
		return tokens
	}
	...
	private func createAdFromConversationCallable(messages: [[String:String]], productImageGsPath: String?) async throws -> (String?, [String: Any]) {
		...
		var inner: [String: Any] = ["messages": messages, "archetype": currentArchetype.rawValue]
	...
	// Allow other views to fetch an ID token without instantiating VM
	static func currentIdTokenStatic() async throws -> String? {
		if let user = Auth.auth().currentUser {
			return try await user.getIDToken()
		}
		return nil
	...
	// Helper to infer projectId (exposed for other views)

    ...
    private func generateCharacter() {
        ...
        Task {
            do {
                ...
                // Build request to our storyboard image generator using Flux context with Pixar style
                let base = "https://us-central1-\(VoiceAssistantVM.projectId()).cloudfunctions.net"
                guard let url = URL(string: "\(base)/generateStoryboardImages") else { throw URLError(.badURL) }
                ...
                let body: [String: Any] = [
                    ...
                    "referenceImageUrls": [uploaded.gsPath ?? uploaded.url],
                    "character": name,
                    "provider": "flux"
                ]
                ...
                let firstUrl = scenes?.first?["imageUrl"] as? String
                ...
                await MainActor.run {
                    ...
                    isGenerating = false
                    // After saving, go straight to Memeverse list
                    let host = UIHostingController(rootView: AdGenChoiceView(presentEditorOnSend: true))

...
struct AdGenChoiceView: View {
    ...
    private func openCreateOriginalCharacter() {
        let host = UIHostingController(rootView: ComingSoonView())
        host.modalPresentationStyle = .overFullScreen
        UIApplication.shared.topMostViewController()?.present(host, animated: true)
    }

    // Simple placeholder screen while we finish the flow
            ...
            ZStack {
                ...
                VStack(spacing: 18) {
                    ...
                    Text("COMING SOON")
                        ...
                        .foregroundColor(.black)
                    Text("Create Original Character is almost ready. We’re adding 3D Pixar-style avatars and instant Memeverse profiles. Check back in a few days!")
